  ...
  useEffect(() => {
    ...
    async function getData() {
      if(!loginData){
        ...
        const response = await fetch('doctor/'+loginData.id+'/details', {headers:{Authorization: `Bearer ${loginData.token}`}});  
        ...
        const response = await fetch('patient/'+loginData.id+'/details', {headers:{Authorization: `Bearer ${loginData.token}`}});
        const data = await response.json();
        setDetails(data);
        setMail(data.email);
        setPhone(data.phoneNumber);
